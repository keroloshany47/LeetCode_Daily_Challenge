...
public:
    long long minCost(vector<int>& basket1, vector<int>& basket2) {
        ...
        # Time: O(n), Space: O(unique_fruits)
       ...
       # Time: O(unique_fruits), Space: O(swaps_needed)
        ...
        # Partial sorting optimization: only sort the cheaper half
        # Time: O(k + (k/2)log(k/2)) where k = toSwap.size()
        int n = toSwap.size();
        if (n == 0) return 0;
